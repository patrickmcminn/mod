  ...
  *new { | freq = 220, amp = 0.2, relGroup = nil, addAction = 'addToTail' |
  ...
  prAddSynthDef {
    SynthDef(\mod_tri, {
      ...
      rangeLo = -1, rangeHi = 1
      |
      var freqMod, ampMod, tri, filter, sig;
      freqMod = InFeedback.ar(freqModIn) * freqModIndex;
      ampMod = Select.ar(ampModFlag, [DC.ar(1), InFeedback.ar(ampModIn)]);
      tri = LFTri.ar(freq + freqMod).range(rangeLo, rangeHi);
      filter = LPF.ar(tri, 20000);
      sig = filter * ampMod;
      sig = sig * amp;
      sig = sig.distort;
      sig = Out.ar(out, sig);
  ...
  prMakeSynth { | freq = 220, amp = 0.2 |
    synth = Synth(\mod_tri, [\out, outBus, \freq, freq, \freqModIn, freqModBus, \ampModIn, ampModBus, \amp, amp],
      group, \addToTail);
  }
